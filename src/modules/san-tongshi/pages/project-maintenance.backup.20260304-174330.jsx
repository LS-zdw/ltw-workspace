...
const mockProjects = [
  { id: "p1", source: "SUIP", projectCode: "H20018", projectName: "17万吨SBC项目", enterpriseName: "海南炼化", buildUnit: "海南巴陵化工新材料有限公司", projectLevel: "一类项目（集团公司级）", projectStage: "可研阶段", buildType: "新建", industry: "炼油", createdAt: "2026-03-02", updatedAt: "2026-03-02" },
  { id: "p2", source: "SUIP", projectCode: "J20402", projectName: "204泊位苯水路出厂项目", enterpriseName: "九江石化", buildUnit: "九江石化", projectLevel: "二类项目（事业部级）", projectStage: "可研阶段", buildType: "新建", industry: "销售", createdAt: "2026-03-03", updatedAt: "2026-03-03" },
  { id: "p3", source: "SUIP", projectCode: "LYS033", projectName: "全厂污水系统达标排", enterpriseName: "洛阳石化", buildUnit: "洛阳石化", projectLevel: "二类项目（事业部级）", projectStage: "竣工验收阶段", buildType: "改建", industry: "化工", createdAt: "2026-03-04", updatedAt: "2026-03-04" },
  { id: "p4", source: "SUIP", projectCode: "MM2026-01", projectName: "乙烯改造项目", enterpriseName: "茂名分公司", buildUnit: "茂名分公司", projectLevel: "二类项目（事业部级）", projectStage: "基础设计阶段", buildType: "改建", industry: "炼化工程", createdAt: "2026-03-05", updatedAt: "2026-03-05" },
  { id: "p5", source: "SUIP", projectCode: "JM2026-08", projectName: "化工罐区优化项目", enterpriseName: "荆门分公司", buildUnit: "荆门分公司", projectLevel: "三类项目（基层级）", projectStage: "可研阶段", buildType: "扩建", industry: "化工", createdAt: "2026-03-06", updatedAt: "2026-03-06" },
  { id: "p6", source: "SUIP", projectCode: "HZ2026-15", projectName: "常减压改造工程", enterpriseName: "海南炼化", buildUnit: "海南炼化", projectLevel: "一类项目（集团公司级）", projectStage: "试运行阶段", buildType: "改建", industry: "石油工程", createdAt: "2026-03-07", updatedAt: "2026-03-07" },
  { id: "p7", source: "MANUAL", projectCode: "MAN-2026-0001", projectName: "智能物联网平台建设项目", enterpriseName: "专业公司", buildUnit: "智能科技建设有限公司", projectLevel: "二类项目（事业部级）", projectStage: "可研阶段", buildType: "新建", industry: "科研", createdAt: "2026-03-08", updatedAt: "2026-03-08" },
  { id: "p8", source: "MANUAL", projectCode: "MAN-2026-0002", projectName: "安环监测一体化项目", enterpriseName: "企业安环处", buildUnit: "信息技术中心", projectLevel: "三类项目（基层级）", projectStage: "可研阶段", buildType: "新建", industry: "其他", createdAt: "2026-03-09", updatedAt: "2026-03-09" },
  { id: "p9", source: "SUIP", projectCode: "WH2026-11", projectName: "危险品仓储升级项目", enterpriseName: "海南炼化", buildUnit: "海南炼化", projectLevel: "二类项目（事业部级）", projectStage: "基础设计阶段", buildType: "扩建", industry: "销售", createdAt: "2026-03-10", updatedAt: "2026-03-10" },
  { id: "p10", source: "SUIP", projectCode: "LY2026-22", projectName: "动力站节能改造", enterpriseName: "洛阳石化", buildUnit: "洛阳石化", projectLevel: "三类项目（基层级）", projectStage: "试运行阶段", buildType: "改建", industry: "炼油", createdAt: "2026-03-11", updatedAt: "2026-03-11" }
];
...
const mockTasks = [
  ...
  { id: "t3", projectId: "p3", status: "项目已完结", createdAt: "2026-03-14", identify: { projectType: "非煤矿山类", specialties: ["消防"], milestones: { designDone: "2026-03-04", trialStart: "2026-06-03", goLive: "2026-09-01", acceptance: "2026-10-31" }, professionalOwners: { safeOwner: "", healthOwner: "", fireOwner: "周岚" } } },
  ...
  { id: "t8", projectId: "p8", status: "项目已完结", createdAt: "2026-03-19", identify: { projectType: "其他", specialties: ["安全", "职卫", "消防"], milestones: { designDone: "2026-03-09", trialStart: "2026-06-08", goLive: "2026-09-06", acceptance: "2026-11-05" }, professionalOwners: { safeOwner: "赵明", healthOwner: "陈伟", fireOwner: "周岚" } } },
  ...
  { id: "t10", projectId: "p10", status: "项目进行中", createdAt: "2026-03-21", identify: { projectType: "其他", specialties: ["职卫", "消防"], milestones: { designDone: "2026-03-11", trialStart: "2026-06-10", goLive: "2026-09-08", acceptance: "2026-11-07" }, professionalOwners: { safeOwner: "", healthOwner: "高远", fireOwner: "刘洋" } } }
];
...
function projectSearchText(project) {
  return [project.projectCode, project.projectName, project.enterpriseName, project.buildUnit].filter(Boolean).join(" ").toLowerCase();
}
...
export default function Page() {
  ...
  const displayRows = React.useMemo(() => {
    ...
    const enterprise = String(topFiltersForm.enterpriseName || "").trim().toLowerCase();
    return rows.filter((r) => {
      ...
      if (enterprise && !String(r.enterpriseName || "").toLowerCase().includes(enterprise)) return false;
  ...
  const filteredProjects = React.useMemo(() => {
    ...
    const enterprise = String(pickFiltersApplied.enterpriseName || "").trim().toLowerCase();
    return projectRows.filter((p) => {
      ...
      if (enterprise && !String(p.enterpriseName || "").toLowerCase().includes(enterprise)) return false;
  ...
  }, [projectRows, pickFiltersApplied]);
    ...
    <div className="stack stpm-page">
      <div className="stpm-filter">
        <div className="stpm-filter-row">
          ...
          <label className="stpm-item"><span>所属企业：</span><input className="stpm-input" placeholder="请输入所属企业" value={topFiltersForm.enterpriseName} onChange={(e) => setTopFiltersForm((p) => ({ ...p, enterpriseName: e.target.value }))} /></label>
        ...
        <div className="stpm-filter-row">
          <label className="stpm-item"><span>任务状态：</span><select className="stpm-input" value={topFiltersForm.taskStatus} onChange={(e) => setTopFiltersForm((p) => ({ ...p, taskStatus: e.target.value }))}><option>全部</option><option>审批中</option><option>审批完成</option><option>项目进行中</option><option>项目已完结</option></select></label>
        ...
        <div className="modal-mask" onClick={() => setShowCreateEntry(false)}>
          <div className="modal modal-xl stpm-create-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-bd detail">
              <div className="filterbar"><div className="filterbar-row"><div className="filterbar-left"><div className="filterbar-item"><div className="filterbar-label">项目来源</div><div className="filterbar-input stpm-multi"><label><input type="radio" checked={createMode === "origin"} onChange={() => setCreateMode("origin")} /> 从项目信息管理选择</label><label><input type="radio" checked={createMode === "manual"} onChange={() => setCreateMode("manual")} /> 手动新增项目</label></div></div></div></div></div>
                ...
                <>
                  <div className="detail-section mt-10" style={{ margin: 0 }}>
                    ...
                    <div className="stpm-create-grid">
                      ...
                      <div className="stpm-create-item"><div className="stpm-create-key"><span className="required-mark">*</span>项目级别</div><div className="stpm-create-val"><select className="filterbar-control" value={manualForm.projectLevel} onChange={(e) => setManualForm((p) => ({ ...p, projectLevel: e.target.value }))}><option value="">请选择项目级别</option><option value="一类项目（集团公司级）">一类项目（集团公司级）</option><option value="二类项目（企业级）">二类项目（企业级）</option><option value="三类项目（基层级）">三类项目（基层级）</option></select></div></div>
                      <div className="stpm-create-item"><div className="stpm-create-key"><span className="required-mark">*</span>项目状态</div><div className="stpm-create-val"><input className="filterbar-control stpm-integrated-control" value="已创建" disabled /></div></div>
      ...
      {showCreateEntry && createMode === "origin" && showPickProjectModal ? (
          ...
          <div className="modal modal-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-bd detail">
              <div className="detail-section" style={{ margin: 0 }}>
                ...
                <div className="stpm-create-grid">
                  <div className="stpm-create-item stpm-create-item-wide"><div className="stpm-create-key">项目检索</div><div className="stpm-create-val"><input className="filterbar-control" value={pickFiltersForm.keyword} placeholder="输入项目编码/项目名称/所属企业/建设单位" onChange={(e) => setPickFiltersForm((p) => ({ ...p, keyword: e.target.value }))} /></div></div>
                  ...
                  <div className="stpm-create-item"><div className="stpm-create-key">所属企业</div><div className="stpm-create-val"><input className="filterbar-control" value={pickFiltersForm.enterpriseName || ""} onChange={(e) => setPickFiltersForm((p) => ({ ...p, enterpriseName: e.target.value }))} /></div></div>
                ...
                <div className="table-wrap mt-10"><table className="proto-table"><thead><tr><th style={{ width: 64 }}>选择</th><th>项目编码</th><th>项目名称</th><th>项目级别</th><th>所属企业</th><th>建设单位</th><th>项目阶段</th><th>项目创建日期</th></tr></thead><tbody>{pageRows.map((project) => (<tr key={project.id} style={{ background: selectedProjectId === project.id ? "#f3f4f6" : undefined }}><td><input type="radio" checked={selectedProjectId === project.id} onChange={() => selectProjectFromPick(project)} /></td><td>{project.projectCode}</td><td>{project.projectName}</td><td>{project.projectLevel}</td><td>{project.enterpriseName}</td><td>{project.buildUnit}</td><td>{project.projectStage}</td><td>{formatDate(project.createdAt)}</td></tr>))}</tbody></table></div>
