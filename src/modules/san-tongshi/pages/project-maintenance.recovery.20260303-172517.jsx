...
const fixedDetailProject = {
  enterpriseName: "茂名分公司",
  projectName: "乙烯改造项目",
  projectCode: "MM2026-01",
  buildUnit: "茂名分公司",
  industry: "化工",
  projectLevel: "二类项目（事业部级）",
  projectStage: "基础设计阶段",
  buildType: "改建",
    ...
    <div className="stack stpm-page">
      ...
      <div className="table-wrap stpm-table-wrap">
        <table className="proto-table">
          <thead>
            <tr>
              <th className="table-checkbox"><input type="checkbox" readOnly /></th><th>序号</th><th>项目名称</th><th>项目建设单位</th><th>项目阶段</th><th>任务状态</th><th>专业分类</th><th>建设类型</th><th>项目类型</th><th>计划开工日期</th><th>计划完工日期</th><th>所属版块</th><th>项目创建日期</th><th>操作</th>
            </tr>
          </thead>
          <tbody>
            {displayRows.map((row) => (
              <tr key={row.id}>
                ...
                <td>{row.order}</td><td>{row.projectName}</td><td>{row.buildOrg}</td><td>{row.projectStage}</td>
              ...
              {createMode === "origin" ? (
                <>
                  <div className="detail-section mt-10" style={{ margin: 0 }}>
                    <div className="detail-section-title">智能检索选择项目</div>
                    ...
                    <div className="table-wrap mt-10"><table className="proto-table"><thead><tr><th style={{ width: 64 }}>选择</th><th>项目编码</th><th>项目名称</th><th>所属企业</th><th>建设单位</th><th>项目阶段</th></tr></thead><tbody>{pageRows.map((project) => (<tr key={project.id} style={{ background: selectedProjectId === project.id ? "#f3f4f6" : undefined }}><td><input type="radio" checked={selectedProjectId === project.id} onChange={() => selectProjectFromPick(project)} /></td><td>{project.projectCode}</td><td>{project.projectName}</td><td>{project.enterpriseName}</td><td>{project.buildUnit}</td><td>{project.projectStage}</td></tr>))}</tbody></table></div>
