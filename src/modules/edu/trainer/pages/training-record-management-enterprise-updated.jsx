...
const certificateDict = {
  国家级: {
    ...
    "特种作业资格证（焊接与热切割作业）": ["熔化焊接与热切割作业", "压力焊作业", "钎焊作业"],
    "安全管理人员资格证": [""],
...
const planExecuteRows = [
  { id: 1, planCode: "ZHLH-2026-001", className: "班组长安全提升一期", period: "1", planPeriods: "2", month: "2", planDays: "6", target: "一线班组长与安全员", content: "安全生产责任制与现场风险辨识", type: "集中面授", hostDept: "安全环保部", undertakeDept: "人力资源部", planPeople: "50", realPeople: "48", passPeople: "46", passRate: "95.8%", certFlag: "是", status: "已提交", recorder: "肖鹏", recordTime: "2026-03-07 17:12:51" },
  { id: 2, planCode: "ZHLH-2026-001", className: "班组长安全提升二期", period: "2", planPeriods: "2", month: "6", planDays: "4", target: "一线班组长与安全员", content: "隐患排查闭环与应急处置演练", type: "集中面授", hostDept: "安全环保部", undertakeDept: "人力资源部", planPeople: "40", realPeople: "39", passPeople: "38", passRate: "97.4%", certFlag: "是", status: "已提交", recorder: "肖鹏", recordTime: "2026-06-21 16:01:28" },
  { id: 3, planCode: "ZHLH-2026-001", className: "班组长案例复盘专题班", period: "2", planPeriods: "2", month: "7", planDays: "2", target: "一线班组长与安全员", content: "典型事故案例复盘与风险预控", type: "线上培训", hostDept: "安全环保部", undertakeDept: "人力资源部", planPeople: "30", realPeople: "29", passPeople: "28", passRate: "96.6%", certFlag: "否", status: "草稿", recorder: "王敏", recordTime: "2026-07-18 14:22:10" }
];
...
function RecordForm({ row, readOnly = false }) {
  ...
  const [selectedPlan, setSelectedPlan] = React.useState(planExecuteRows.find((item) => item.className === row.className) || planExecuteRows[0]);
  const [certFlag, setCertFlag] = React.useState(row.certFlag || "否");
  ...
  const actualDays = calcRecordDays(row);
  const showCertificateFields = certFlag === "是";
  const submittedPlans = planExecuteRows.filter((item) => item.status === "已提交");
    ...
    <>
      <div className="cert-section">
        ...
        <div className="cert-form-grid">
          <Field label="关联培训计划" required wide>
            <div className="record-plan-picker">
              <input className="cert-field-control" value={`${selectedPlan.planCode} ${selectedPlan.className}`} readOnly />
              {!readOnly ? <button type="button" className="btn" onClick={() => setPlanModalOpen(true)}>选择培训计划</button> : null}
            </div>
          </Field>
          <Field label="本次期次" required><input className="cert-field-control" defaultValue={selectedPlan.period} readOnly={readOnly} placeholder="请输入本次期次" /></Field>
          <Field label="计划天数"><input className="cert-field-control" value={selectedPlan.planDays} readOnly /></Field>
          ...
          <Field label="实际天数"><input className="cert-field-control" value={actualDays} readOnly /></Field>
          <Field label="培训内容" required wide>
            <textarea
              className="cert-field-control cert-field-textarea cert-field-textarea-large"
              defaultValue={selectedPlan.content}
              readOnly={readOnly}
              placeholder="请输入培训内容"
            />
          </Field>
          <Field label="是否取证" required>
            <div className="filterbar-radio-group">
              <label className="filterbar-radio">
                <input type="radio" name={`certFlag-${row.id || "new"}`} value="是" checked={certFlag === "是"} disabled={readOnly} onChange={(e) => setCertFlag(e.target.value)} />
                是
              </label>
              <label className="filterbar-radio">
                <input type="radio" name={`certFlag-${row.id || "new"}`} value="否" checked={certFlag === "否"} disabled={readOnly} onChange={(e) => setCertFlag(e.target.value)} />
                否
              </label>
            </div>
          </Field>
          {showCertificateFields ? (
            <>
          <Field label="发证机关级别" required>
            <select className="cert-field-control" value={formCertLevel} disabled={readOnly} onChange={(e) => updateFormCertLevel(e.target.value)}>
              {Object.keys(certificateDict).map((level) => <option key={level} value={level}>{certificateLevelLabels[level]}</option>)}
            </select>
          </Field>
          <Field label="证书种类" required>
            <select className="cert-field-control" value={formCertType} disabled={readOnly} onChange={(e) => updateFormCertType(e.target.value)}>
              {Object.keys(certificateDict[formCertLevel] || {}).map((type) => <option key={type} value={type}>{type}</option>)}
            </select>
          </Field>
          <Field label="证书小类" required>
            <select className="cert-field-control" value={formCertSubtype} disabled={readOnly} onChange={(e) => setFormCertSubtype(e.target.value)}>
      ...
      {planModalOpen ? (
        <div className="modal-mask" onClick={() => setPlanModalOpen(false)}>
          <div className="modal modal-xl cert-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-hd">
              <div className="modal-title">选择培训计划</div>
              <button type="button" className="modal-close" onClick={() => setPlanModalOpen(false)}>x</button>
            </div>
            <div className="modal-bd cert-bd">
              <div className="cert-section">
                <div className="cert-section-title">已提交培训计划</div>
                <div className="record-plan-tip">仅可选择状态为“已提交”的培训计划。</div>
                <div className="table-wrap">
                  <table className="proto-table">
                    <thead>
                      <tr><th>序号</th><th>项目代码</th><th>项目名称</th><th>期次</th><th>计划天数</th><th>主办部门</th><th>承办部门</th><th>是否取证</th><th>状态</th><th>操作</th></tr>
                    </thead>
                    <tbody>
                      {submittedPlans.map((plan, index) => (
                        <tr key={plan.id}>
                          <td>{index + 1}</td><td>{plan.planCode}</td><td>{plan.className}</td><td>{plan.planPeriods}</td><td>{plan.planDays}</td><td>{plan.hostDept}</td><td>{plan.undertakeDept}</td><td>{plan.certFlag}</td><td>{plan.status}</td>
                          <td><button type="button" className="table-link-btn" onClick={() => { setSelectedPlan(plan); setCertFlag(plan.certFlag); setPlanModalOpen(false); }}>选择</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="modal-ft"><button type="button" className="btn" onClick={() => setPlanModalOpen(false)}>关闭</button></div>
          </div>
        </div>
      ) : null}
            ...
            <table className="proto-table">
              <thead>
                <tr>
                  ...
                  <th>序号</th><th>项目代码</th><th>项目名称</th><th>期次</th><th>计划天数</th><th>实际天数</th><th>实际培训日期</th><th>培训内容</th><th>参训人数</th><th>合格人数</th><th>合格率</th><th>是否取证</th><th>状态</th><th>登记人</th><th>登记部门</th><th>登记日期</th><th>操作</th>
                </tr>
              </thead>
              <tbody>
                {recordRows.map((row) => (
      ...
      {activeModal === "add-record" ? (
        <div className="modal-mask" onClick={() => setActiveModal("")}> 
          <div className="modal modal-xl cert-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-ft"><button type="button" className="btn" onClick={() => setActiveModal("")}>取消</button><button type="button" className="btn btn-primary">保存</button><button type="button" className="btn btn-primary">提交</button></div>
          </div>
        </div>
      ) : null}
